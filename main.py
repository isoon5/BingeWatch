...
def _show(data, season, episode, flag):  
    ...
    def update_score(id, rating):
        print('Rating: ', rating)
        
        c = conn.cursor()
        c.execute(""" UPDATE tv_shows SET rated_episodes = rated_episodes + 1 WHERE id LIKE ? """, (id,))
        conn.commit()

        c.execute(""" SELECT  rated_episodes from tv_shows WHERE id LIKE ? """, (id,))
        string = ''.join(str(x) for x in c.fetchall())
        rated_episodes = int(string[1])
    ...
    submitButton = Button(root, text="Rate", command = lambda: update_score(data[0],rating.get()) , width=5, font = ('Verdana', 10), bg = '#696969')
...
def favorites():
    ...
    c = conn.cursor()
    row = c.execute("select * from tv_shows where finished = 0")
    rows = c.fetchall()
    endline = 0
    photoimage_list.clear() 
    vertical_pos = 0
